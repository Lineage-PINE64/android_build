...
""" % bcb_dev)
  ...
  boot_resource_fex = common.GetFex("boot-resource.fex",OPTIONS.target_tmp+str("/boot-resource.fex"))
  common.ZipWriteStr(output_zip, "boot-resource.fex", boot_resource_fex.data)
  script.WriteRawFex("/dev/block/by-name/bootloader", "boot-resource.fex")
  ...
  uboot = common.GetFex("u-boot.fex", OPTIONS.target_tmp + str("/u-boot.fex"))
  common.ZipWriteStr(output_zip, "u-boot.fex", uboot.data)
  script.BurnBoot(1);
  ...
  boot0_sdcard = common.GetFex("boot0_sdcard.fex", OPTIONS.target_tmp + str("/boot0_sdcard.fex"))
  common.ZipWriteStr(output_zip, "boot0_sdcard.fex", boot0_sdcard.data)
  script.BurnBoot(0);
...
def WriteIncrementalOTAPackage(target_zip, source_zip, output_zip):
  ...
  source_boot_resource = common.GetFex("boot-resource.fex",OPTIONS.source_tmp+str("/boot-resource.fex"))
  target_boot_resource = common.GetFex("boot-resource.fex",OPTIONS.target_tmp+str("/boot-resource.fex"))
  updating_boot_resource = (source_boot_resource.data != target_boot_resource.data)
  ...
  source_uboot = common.GetFex("u-boot.fex", OPTIONS.source_tmp + str("/u-boot.fex"))
  target_uboot = common.GetFex("u-boot.fex", OPTIONS.target_tmp + str("/u-boot.fex"))
  updating_uboot = (source_uboot.data != target_uboot.data)
...
""" % bcb_dev)
  ...
  if updating_boot_resource:
    print "boot-resource fex changed; including."
    common.ZipWriteStr(output_zip, "boot-resource.fex", target_boot_resource.data)
    script.WriteRawFex("/dev/block/by-name/bootloader", "boot-resource.fex")
  else:
    print "boot-resource fex unchanged; skipping."
  ...
  if updating_uboot:
    ...
    common.ZipWriteStr(output_zip, "u-boot.fex", target_uboot.data)
    script.BurnBoot(1);
  else:
